...
2. `codeguard-init`은 별도 승인 signal이나 자식 프로세스 없이 기존 사용자 프로그램을 `exec`한다.
3. `strace`는 사용자 프로그램 종료와 trace footer 기록을 마친 뒤 종료한다.
4. Runner는 Execution cgroup과 Root TID를 native tracker에 등록하고 종료 후 Snapshot을 회수한다.
