...
class TaskTrackerIntegrationTests(unittest.TestCase):
    ...
    def assert_snapshot(self, response, processes: int, threads: int) -> None:
        ...
        self.assertEqual(
            response.resource_usage.process_at_pids_peak,
            processes,
        )
        self.assertEqual(
            response.resource_usage.thread_at_pids_peak,
            threads,
        )

    def test_single_task_snapshot_is_stable_across_30_runs(self) -> None:
        for _ in range(30):
            self.assert_snapshot(self.execute(SINGLE_TASK_SOURCE), 1, 0)

    def test_thread_barrier_snapshot(self) -> None:
        self.assert_snapshot(self.execute(THREAD_BARRIER_SOURCE), 1, 15)
    ...
    def test_short_lived_threads_are_not_missed(self) -> None:
        for _ in range(30):
            self.assert_snapshot(self.execute(SHORT_THREADS_SOURCE), 1, 1)

    def test_short_lived_processes_are_not_missed(self) -> None:
        for _ in range(30):
            self.assert_snapshot(self.execute(SHORT_PROCESSES_SOURCE), 2, 0)

    def test_main_thread_exit_keeps_process_identity(self) -> None:
        self.assert_snapshot(self.execute(MAIN_THREAD_EXITS_SOURCE), 1, 2)
    ...
    def test_concurrent_runs_do_not_mix_metrics(self) -> None:
        ...
        expected = [(1, 0), (1, 15)] * 5
        for response, (processes, threads) in zip(responses, expected):
            self.assert_snapshot(response, processes, threads)
